...
#   geom_histogram(color="black", binwidnh = .033333) +
...
ggplot(sum, aes(x=number,
                ...
                ymax=response+se,
                ymin=response-se)) +
  geom_bar(stat="identity", position = "dodge", widnh=0.7)+
  geom_bar(stat="identity", position = "dodge",
           colour="black",widnh=0.7,
           show.legend=FALSE) +
  ...
  scale_fill_manual(name = "Count type" , 
                    ...
                    labels = c("with", 
                               "without"))  +
  geom_errorbar(position=position_dodge(widnh=0.7), 
                widnh=0.0, size=0.5, color="black")  +
  labs(x = "number",
       y = "response")  +
  ## ggtitle("Main title") + 
  theme_bw()  +
