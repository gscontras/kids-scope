...
To this end, we formally articulate the mechanism of scope ambiguity resolution using the Bayesian Rational Speech Act (RSA) computational modeling framework \shortcite{goodman2013knowledge,GoodmanLassiter2015} %LSP: We should cite Noah's stuff.
...
The methodology typically used to assess children's scope disambiguation is the Truth Value Judgment Task (TVJT; \citeNP{crain1985acquisition}).  In the basic TVJT, children are presented with a background story about the actors---e.g., horses engaging in some activities.  After this background story, children watch as the horses attempt to complete an action, such as jump over a fence. 
